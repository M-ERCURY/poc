...
func Cmd() *cli.Subcmd {
	...
	r.Run = func(fm fsdir.T) {
		...
		if c.Contract == nil {
			...
			log.Fatalf(
				"you are trying to import accesskeys for a contract %s different from the currently defined %s, please set up a separate mercury directory for your %s needs and import %s there",
